...
$( function() {
    ...
    var width, height, canvas, ctx, points, randColor, target, animateHeader = true;
    ...
    function initHeader() {
        ...
        // assign a circle to each point Circle(position, radius seed, color) sets size range
        for ( var i in points ) {
            //var c = new Circle( points[ i ], 4 + Math.random() * 12, 'rgba(221, 221, 221,0.3)' );
            var c = new Circle( points[ i ], 2 + Math.random() * 15, 'rgba(255, 79, 0,0.3)' );
            //var c = new Circle( points[ i ], 4 + Math.random() * 20, randColor );
            points[ i ].circle = c;
        }
    }
    ...
    function Circle( pos, rad, color ) {
        var randColor = randomColor({format:'rgba', hue: 'bright'});
        var _this = this;
        ...
        /* orginal draws circles
        this.draw = function() {
            if ( !_this.active )
                return;
            ctx.beginPath();
            ctx.arc( _this.pos.x, _this.pos.y, _this.radius, 0, 2 * Math.PI, false );
            //ctx.fillStyle = 'rgba(255, 79, 0,' + _this.active + ')';
            //ctx.fillStyle = randColor;
            ctx.fillStyle = (randColor.substr(0,randColor.search(", 0."))+ ', ' + _this.active + ')');
            ctx.fill();
        };
        */
        this.draw = function() {
            ...
            var x = _this.pos.x;
            var y = _this.pos.y;
            var w = _this.radius*4;
            var h = _this.radius*2;
            ...
            ctx.arc( _this.pos.x, _this.pos.y, _this.radius / 2, 0, 2 * Math.PI, false ); // Michael Moore made radius smaller
            //ctx.fillStyle = 'rgba(221, 221, 221,' + _this.active + ')';
            //ctx.fillStyle = 'rgba(255, 79, 0,' + _this.active + ')';
            ctx.fillStyle = (randColor.substr(0,randColor.search(", 0."))+ ', ' + _this.active + ')');
            ctx.fill();
            ...
            ctx.beginPath();
            ctx.moveTo(x,y);
            ctx.lineTo(x+ radius, y-radius); //left callout
            ctx.quadraticCurveTo(x,y-radius,x,y-h); //6 to 9

            ctx.lineTo(x,y-h); //right vert

            ctx.quadraticCurveTo(x,y-radius-h,x+radius,y-radius-h); // 9 to 12

            ctx.lineTo(x+w-radius,y-radius-h); //top line
            ctx.quadraticCurveTo(x+w,y-radius-h,x+w,y-h); //12 to 3

            ctx.lineTo(x+w,y-h); //left vert

            ctx.quadraticCurveTo(x+w,y-radius,x+w-radius,y-radius); //3 to 6
            ctx.lineTo(x+h,y-radius); // bottom line
            ctx.lineTo(x,y); // right callout
